...
import { Canvas2DSource, CanvasMode } from "./canvas2d-source";
  ...
  setupEventListeners() {
    ...
    document.getElementById("add-strip")?.addEventListener("click", () => {
      const count = parseInt(
        (document.getElementById("strip-count") as HTMLInputElement)?.value,
      );
    ...
    document.getElementById("add-grid")?.addEventListener("click", () => {
      const rows = parseInt(
        (document.getElementById("grid-rows") as HTMLInputElement)?.value,
      );
      const cols = parseInt(
        (document.getElementById("grid-cols") as HTMLInputElement)?.value,
      );
    ...
    if (samplesInput) {
      samplesInput.addEventListener("change", () => {
        const n = parseInt(samplesInput.value as string);
