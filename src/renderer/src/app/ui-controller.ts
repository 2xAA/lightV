...
export class UIController {
  ...
  currentEditRegion: null | Region;
  onRegionEdit: (regionId: string) => void;
  onRegionDelete: (regionId: string) => void;
  currentOnSave: (updatedConfig: any) => void;
  ...
  init() {
    ...
    this.editModal = document.getElementById("edit-modal") as HTMLDialogElement;
  ...
  updateEmptyState() {
    ...
    resultsTable.style.display = hasRows ? "table" : "none";
  }
  ...
  showEditModal(region, onSave) {
    ...
    if (!modalTitle || !modalControls) return;
  ...
  saveModalChanges() {
    ...
    switch (this.currentEditRegion.type) {
      case "strip": {
        const stripCount = document.getElementById(
          "modal-strip-count",
        ) as HTMLInputElement;
        if (stripCount) {
      ...
      case "grid": {
        const gridRows = document.getElementById(
          "modal-grid-rows",
        ) as HTMLInputElement;
        const gridCols = document.getElementById(
          "modal-grid-cols",
        ) as HTMLInputElement;
