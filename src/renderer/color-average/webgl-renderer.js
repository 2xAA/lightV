...
export class WebGLRenderer {
  ...
  renderSource() {
    ...
    this.gl.uniform1i(modeLocation, 0); // Display mode
    this.gl.uniform1i(regionCountLocation, 0);
    this.gl.uniform1i(statLocation, 0);
    this.gl.uniform1i(samplesLocation, this._samplesPerEdge);
  ...
  calculateAverageColors(regionBounds, orientedQuads = null) {
    return this.calculateColors(regionBounds, orientedQuads, "average");
  }

  calculateColors(regionBounds, orientedQuads = null, method = "average") {
    if (
    ...
    this.gl.bindTexture(this.gl.TEXTURE_2D, this.sourceTexture);
    this.gl.uniform1i(imageLocation, 0);
    this.gl.uniform1i(modeLocation, 1);
    this.gl.uniform1i(regionCountLocation, count);
    this.gl.uniform2f(canvasSizeLocation, canvasWidth, canvasHeight);
    this.gl.uniform1i(useOrientedLocation, useOriented ? 1 : 0);
    this.gl.uniform1i(
      statLocation,
    ...
    this.gl.uniform1i(samplesLocation, this._samplesPerEdge);
    if (useOriented) {
      this.gl.uniform4fv(p0Location, p0Data);
      this.gl.uniform4fv(uLocation, uData);
      this.gl.uniform4fv(vLocation, vData);
    } else {
      this.gl.uniform4fv(regionsLocation, regionData);
    }
